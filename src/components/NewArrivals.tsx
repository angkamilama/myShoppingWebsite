...
function newArrivals({ fetchedData }: { fetchedData: Item[] }) {
  ...
  const newArrivalsData = fetchedData.slice(12, 17);
  return (
    <Swiper
      slidesPerView={2}
      spaceBetween={10}
    ...
    >
      {newArrivalsData.map((item) => {
        ...
        return (
          <SwiperSlide key={id}>
            <div
              className="flex flex-col justify-evenly items-center mt-1 bg-white relative mb-8 p-4 h-[300px] text-center"
              // onMouseEnter={() => setShowProductOption(true)}
              // onMouseLeave={() => setShowProductOption(false)}
              onMouseOverCapture={() => setShowProductOption(true)}
            >
              <img
                src={image}
